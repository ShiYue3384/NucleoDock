...
from ..modeling.modeling_foldock2_all import FoldForDocking
...
def args_parse(config_file=''):
    ...
    # cfg = get_config(config_file, args.opts) #ls revise
    cfg = get_config(config_file)  ## ls revise
    if len(cfg.OUTPUT_DIR) == 0:
        cfg.OUTPUT_DIR = get_abs_path('outputs', cfg.MODEL.NAME, cfg.MODEL.TASK, str(datetime.date.today()))
    else:
        cfg.OUTPUT_DIR = get_abs_path(cfg.OUTPUT_DIR, cfg.MODEL.NAME, cfg.MODEL.TASK, str(datetime.date.today()))
...
def load_checkpoint(cls, cfg, **kwargs):
    if not cfg.MODEL.IS_ORIGIN:
        model_config = UnimolConfig.from_pretrained(cfg.MODEL.CHECKPOINT_PATH,
                                                    num_labels=cfg.MODEL.NUM_CLASSES,
                                                    pooler_dropout=cfg.MODEL.DROPOUT,
                                                    hidden_dropout_prob=cfg.MODEL.DROPOUT,
                                                    attention_probs_dropout_prob=cfg.MODEL.DROPOUT,
                                                    **kwargs)
        print(model_config)
        return cls.from_pretrained(cfg.MODEL.CHECKPOINT_PATH, config=model_config)
    model_config = UnimolConfig(vocab_size=31,
                                num_labels=cfg.MODEL.NUM_CLASSES,
                                pooler_dropout=cfg.MODEL.DROPOUT,
                                hidden_dropout_prob=cfg.MODEL.DROPOUT,
                                attention_probs_dropout_prob=cfg.MODEL.DROPOUT,
    ...
    for k, v in torch.load(cfg.MODEL.CHECKPOINT_PATH)['model'].items():
        if 'pair2coord_proj' not in k:
            state_dict[f'unimol.{k}'] = v
        else:
            state_dict[k] = v
