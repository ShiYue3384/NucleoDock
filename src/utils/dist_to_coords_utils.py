...
def get_mask_rotate(mol, device='cpu'):
    ...
    ## ls revise  #If it is a multi-fragment molecule, only keep the largest one
    ...
    # ls revise对于 0 原子/0 边的奇怪情况，直接返回空 torsions/masks
    #if G.number_of_nodes() == 0 or G.number_of_edges() == 0:
        #return [], []

    # 如果还是不连通（理论上不太可能），我们也直接返回空 torsions/masks，避免整个流程挂掉
    if not nx.is_connected(G):
        # 以前这里是 assert，现在改成安全返回
        assert nx.is_connected(G), "The molecular graph must be a connected graph"
        #return [], []
    #assert nx.is_connected(G), "分子图必须为连通图"  #ls revise ended
    torsions = []
    masks = []
