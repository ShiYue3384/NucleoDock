...
class MultiProcess:
    ...
    def dist_to_coords_with_tor(self, init_coord, pred_cross_dist, pred_holo_dist):
        ...
        for i in range(self.iterations):
            def closure():  ##跑的是这一部分，但这只是在评判距离矩阵是否可靠
                optimizer.zero_grad()
        ...
        if best_loss < 100:
            ...
            opt_mol = add_coord(opt_mol, self.pocket_center.cpu().data.numpy())
            opt_mol.SetProp('loss', f'{best_loss}')  #经过tor的loss
            opt_mol.SetProp('score', f'{score}')
            return opt_mol, best_loss, score
...
def convert_dist2coord(infer_output, ligands: List, target_mol=None, output_path=None, num_threads=1, lbfgsbsrv=None):
    ...
    if lbfgsbsrv is not None:
        ...
        if num_threads == -1:
            ...
            mol_list = [mp.dist_to_coords_with_tor(*arg) for arg in args]  ##真正慢的是这一步
    mol_list = sorted([m for m in mol_list if m is not None], key=lambda x: 5 * x[1] - 1 * x[2])  ##z=这里的排序是loss-score,所以应该是越小越好，默认升序
    #mol_list = sorted([m for m in mol_list if m is not None], key=lambda x: x[1])
...
def mdn_score(pi, mu, sigma, predict_coords=None, pocket_coords=None, dist=None, threshold=5, reduction='sum'):  ##“怎么把一堆数压缩成一个标量”
    if dist is None:
        dist = torch.norm(predict_coords.unsqueeze(1) - pocket_coords.unsqueeze(0), dim=-1)
    dist_mask = dist < threshold   
    # ##ls debug
    # dist_mask_np = dist_mask.detach().cpu().numpy().astype(int)

    # # 写入文本文件
    # with open("dist_mask.txt", "w") as f:
    #     if dist_mask_np.ndim == 2:
    #         # 形状 [N, M]，直接写一张 0/1 矩阵
    #         np.savetxt(f, dist_mask_np, fmt="%d")
    #     elif dist_mask_np.ndim == 3:
    #         # 形状 [B, N, M]，每个 batch 一个 block
    #         B = dist_mask_np.shape[0]
    #         for b in range(B):
    #             f.write(f"# sample {b}\n")
    #             np.savetxt(f, dist_mask_np[b], fmt="%d")
    #             f.write("\n")
    #     else:
    #         # 其他情况就先 flatten 再写
    #         flat = dist_mask_np.reshape(dist_mask_np.shape[0], -1)
    #         np.savetxt(f, flat, fmt="%d")
    #     ##--ls debug
    normal = Normal(mu, sigma)  #
    # [BSZ, N, M, 10]
    loglik = normal.log_prob(dist.unsqueeze(-1)) #对每个 (i,j,k) 分量，算在该距离下的 log-likelihood
    logprob = loglik + torch.log(pi)  # 对每个分量加上混合权重的 log
    # [BSZ, N, M]
    prob = logprob.exp().sum(-1)   ##对 k 维求和，得到“混合分布下该距离的 pdf 值
    if reduction == 'mean':  ##所以reduction是啥意思
...
def read_ligands(mol_list=None, smiles=None, num_gen_conf=100, num_use_conf=6):   ##这参数完全没用到吧num_gen_conf
    """
    ...
    total_mol_list = [gen_init_conf(mol, num_confs=num_use_conf) for mol in mol_list]  ##聚类生成num_use_conf个构象
...
def model_inference(model, pocket, ligands: List, ligand_dict, pocket_dict, device='cuda', bsz=8):
    ...
    # print('prepare data...')
    l_data, p_data = [], []  #l_data 用来存每个 batch 对应的 ligand 输入
    for i in range(int(math.ceil(ligand_nums / bsz))):  #按 bsz 把 ligands 切成块，并为每一块准备 ligands/pocket 的模型输入。我觉得设置为8没有问题
        l_data.append(
    ...
    with torch.no_grad():
        ...
        distance_predict_tta = torch.cat([output.cross_distance_predict for output in outputs])[:, 1:-1, 1:-1]  #把头尾两个位置裁掉，通常是去掉 [CLS]/[PAD]/[EOS] 之类的特殊 token 对应的行/列,但是裁掉的位置大多都是1.好奇怪
        # ##ls debug
        # import numpy as np
        # dist_np = distance_predict_tta.detach().cpu().numpy()  # shape: (B, N_lig, N_poc)
        # with open("distance_predict_tta.txt", "w") as f:
        #     B = dist_np.shape[0]
        #     for b in range(B):
        #         f.write(f"# sample {b}\n")  # 标记是哪一个 TTA / 样本
        #         np.savetxt(f, dist_np[b], fmt="%.4f")
        #         f.write("\n")  # block 之间空一行
        #print(distance_predict_tta)  
        ## debug
        ...
        mean_cross_dist = torch.mean(distance_predict_tta, dim=0)  ##完全没问题
        # # ls debug
        # mean_cross_dist_np = mean_cross_dist.detach().cpu().numpy()  # shape: [N_lig, N_poc]
        # # 写入到文本文件 mean_cross_dist.txt
        # with open("mean_cross_dist.txt", "w") as f:
        #     np.savetxt(f, mean_cross_dist_np, fmt="%.6f")
        # ##ls debug    
        score = mdn_score(pi, mu, sigma, dist=mean_cross_dist).item() ##debug一下mdn_score部分
...
#ls revise
    ...
    print(f"[PROFILE][docking] model_inference: {t1_infer - t0_infer:.3f} s, "
          f"convert_dist2coord: {t1_convert - t0_convert:.3f} s")
    # ls revise end
    rst = dict()
    rst['mol_list'] = rdkit_mol_list
    rst['conf_ranking_score'] = [5 * item[1] - item[2] for item in mol_list]  ##所以是loss-score 越小越好
